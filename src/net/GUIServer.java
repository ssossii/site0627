...
public class GUIServer extends JFrame{
   ...
   Thread thread;// 지연, 대기,루프 등에 사용하기 위함!~!
                  //왜 why? 메인 쓰레드는 중요하니깐.. 운영자니깐..
   ...
   private void startServer() {
      ...
      try {
         ...
         //자바는 쓰레드 기반이기에, 우리가 실행부라고 불렀던, 그 실행주체가 바로 main thread였다
         //메인 쓰레드의 역할은 프로그램을 운영하는 역할
         //특히 GUI 프로그램에서는 사용자의 이벤트 감지, 그래픽 랜더링 등등...
         //따라서 메인 쓰레드를 대기, 루프, 지연 상태에 빠트리면, 프로그램자체가 불통
         //심지어 안드로이드에서는 컴파일 에러 대상이다
         String ip=client.getInetAddress().getHostAddress();//InetAddress ip 주소를 얻어낼 수 있따
         area.append(ip+"님 접속\n");
