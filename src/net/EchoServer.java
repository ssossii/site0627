//에코서버란, 클라이언트의 메세지를 받은 후, 그대로 전달하는 역할의 네트워크
//서버를 의미!!(채팅 제작의 기초)
//소켓이란 ?? 전산에서의 소켓도 현실에서의 소켓처럼 이미 내부적으로 
//            기술적인 부분을 구현해놓은 구현체를 말한다..
//            특히 복잡한 부분을 추상화 시켜놓아 개발자가 메서드호출만으로도
//            기술구현이 가능하도록 해놓음...
//자바네트워크에서의 소켓은 2종류
...
public class EchoServer {
   ...
   public EchoServer() {
      try {
         ...
         //Socket은 대화를 나눌 수 있는 전화기와 같다!!
         Socket client=server.accept();         //접속자 기다림..메인쓰레드인 실행부는
                                                   //접속자가 발생할때까지 대기상태에 빠짐..지연...
         System.out.println("접속자 발견!!");
         //클라이언트가 보낸 메세지 받으려면.. 실행중인 프로그램으로
         //데이터가 들어오는 것이므로, 입력스트림이 필요하다!!
         ...
         while(true) {
            String data=buffr.readLine();// read는 바이트기반으로 읽어들인다
